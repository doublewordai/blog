...
export default function SpecDecOptimalGamma() {
  ...
  // Create (and rebuild only on theme). Slider changes update the data in place
  // below, so the line morphs instead of replaying the mount animation from zero.
  useEffect(() => {
  ...
  return (
    <div className="my-6">
      ...
      >
        <div style={row}>
          <label style={{whiteSpace: 'nowrap'}}>avg seq len</label>
          <input
            type="range"
            min={0}
            max={100}
            ...
            style={{width: '150px'}}
          />
          <span style={{fontVariantNumeric: 'tabular-nums', minWidth: '4.5rem'}}>
            {seqLen.toLocaleString()} tok
          </span>
        </div>
        <div style={row}>
          <label style={{whiteSpace: 'nowrap'}}>acceptance α</label>
          <input
            type="range"
            min={1}
            max={99}
            ...
            style={{width: '150px'}}
          />
          <span style={{fontVariantNumeric: 'tabular-nums', minWidth: '4.5rem'}}>{alphaPct}%</span>
        </div>
        <div style={row}>
          <label style={{whiteSpace: 'nowrap'}}>drafter cost</label>
          <input
            type="range"
            min={0}
            max={50}
            ...
            style={{width: '150px'}}
          />
          <span style={{fontVariantNumeric: 'tabular-nums', minWidth: '4.5rem'}}>{cDraftPct}%</span>
        </div>
      </div>
