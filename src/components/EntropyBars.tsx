...
import {models, ALL_FORMATS, type Model} from './weight-entropy-models'
...
export default function EntropyBars({title = '', defaultFormats = ['BF16']}: Props) {
  const chartRef = useRef<HTMLCanvasElement>(null)
  const chartInstance = useRef<Chart | null>(null)
  const [active, setActive] = useState<Set<string>>(new Set(defaultFormats))
  const theme = useChartTheme()

  const visible = useMemo(() => models.filter((m) => active.has(m.format)), [active])
  ...
  useEffect(() => {
    ...
    const sortedWidths = Object.keys(groups)
      .map(Number)
      .sort((a, b) => a - b)
      ...
      data: {
        ...
        datasets: [
          ...
          {label: 'Scale (amortized)', data: scaleUsed, backgroundColor: '#8b5cf6', borderWidth: 0},
          {
            ...
            borderWidth: 0,
            datalabels: {
              display: (ctx: {dataIndex: number}) => modelAtIdx[ctx.dataIndex] !== null,
              anchor: 'end' as const,
              align: 'end' as const,
              formatter: (_: number, ctx: {dataIndex: number}) => {
