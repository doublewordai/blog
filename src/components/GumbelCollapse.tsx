...
import {models, ALL_FORMATS} from './weight-entropy-models'
...
export default function GumbelCollapse({title = '', defaultFormats = ['BF16', 'FP8', 'MXFP8']}: Props) {
  ...
  useEffect(() => {
    ...
    const datasets: ChartDataset<'scatter'>[] = visible.map((m) => {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) =>
                `${ctx.dataset.label}: ${(ctx.parsed.y ?? 0).toFixed(2)}% at z=${(ctx.parsed.x ?? 0).toFixed(2)}`,
            },
          },
        },
        scales: {
          x: {
            title: {display: true, text: 'Normalized magnitude: (log₂|w| − μ) / β'},
            min: -10,
            max: 4,
